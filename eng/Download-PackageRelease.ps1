...
single atomic step. That folder is declared in src/NuGet.Config, so a plain `dotnet restore` picks
the packages up while everything else continues to come from the corporate proxy.
...
$VersionProjectPath = Join-Path $RepositoryRoot 'src' 'Diginsight.Components' 'Diginsight.Components.csproj'
...
Write-Host ''
Write-Host 'Run: dotnet restore src/Diginsight.Components.Build.slnx --force-evaluate'
