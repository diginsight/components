...
try {
    ...
    Invoke-Test 'automatic build versions normalize unchanged' {
        Assert-Equal '0.8.0.123' (ConvertTo-NormalizedPackageVersion -Version 'v0.8.0.123' -SourceTag)
        Assert-Equal '0.8.0' (ConvertTo-NormalizedPackageVersion -Version 'v0.8.0.0' -SourceTag)
    }

    Invoke-Test 'the shipped package inventory lists the eight component packages' {
        $configuration = Get-PackageConfiguration (Join-Path $PSScriptRoot '..' 'package-manifest.json')
        Assert-Equal 'diginsight/components' $configuration.Repository
        Assert-Equal 'src/Diginsight.Components.Build.slnx' $configuration.Solution
        Assert-Equal 8 $configuration.Packages.Count
        foreach ($package in $configuration.Packages) {
            if (-not $package.Id.StartsWith('Diginsight.Components', [System.StringComparison]::Ordinal)) {
                throw "Unexpected package id '$($package.Id)'."
            }
            if (-not $package.SymbolsRequired) {
                throw "Package '$($package.Id)' must require symbols."
            }
        }
    }
