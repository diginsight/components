...
{
    private static readonly Type T = typeof(ObservabilityExtensions);
    ...
    )
    {
        ...
        if (configureDefaults)
        {
            services.Configure<DiginsightActivitiesOptions>(
                dao =>
                {
                    ...
                    dao.SpanDurationMeterName = assemblyName;
                }
            );
        ...
        logger.LogDebug("openTelemetryOptions.EnableMetrics: {EnableMetrics}", openTelemetryOptions.EnableMetrics);
        if (openTelemetryOptions.EnableMetrics)
        {
            ...
            var defaultMetricTags = diginsightConfig.GetSection("MetricTags").Get<string[]>() ?? [ ];
            ...
            foreach (var metricName in metricNames)
            {
                services.Configure<MetricRecordingNameBasedFilterOptions>(metricName, options =>
                {
                    ...
                    if (metricConfig is not null) { activitiesToUse.AddRange(metricConfig.ActivityNames); }
                ...
                services.Configure<MetricRecordingEnricherOptions>(metricName, options =>
                {
                    ...
                    if (metricConfig is not null) { tagsToUse.AddRange(metricConfig.MetricTags); }
                ...
                services.AddNamedSingleton<IMetricRecordingFilter, MetricRecordingNameBasedFilter>(
                    metricName, (sp, key) =>
                    {
                        ...
                        return new MetricRecordingNameBasedFilter(namedOptsionsMonitor);
                    }
                );
                services.AddNamedSingleton<IMetricRecordingEnricher, MetricRecordingTagsEnricher>(metricName, (sp, key) =>
                {
                    ...
                    return new MetricRecordingTagsEnricher(namedOptsionsMonitor);
                });
            }
        ...
        logger.LogDebug("openTelemetryOptions.EnableTraces: {EnableTraces}", openTelemetryOptions.EnableTraces);
