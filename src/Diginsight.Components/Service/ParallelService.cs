...
public class ParallelService : IParallelService
{
    ...
    public int LowConcurrency
    {
        get
        {
            if (lowConcurrency is not null) { return lowConcurrency.Value; }
    ...
    public int MediumConcurrency
    {
        get
        {
            if (mediumConcurrency is not null) { return mediumConcurrency.Value; }
    ...
    public int HighConcurrency
    {
        get
        {
            if (highConcurrency is not null) { return highConcurrency.Value; }
    ...
    async Task IParallelService.WhenAllAsync(IEnumerable<Func<Task>> taskFactories, ParallelOptions parallelOptions)
    {
        ...
        await ForEachAsync(taskFactories, parallelOptions, static taskFactory => taskFactory());
    }
    ...
    {
        using var activity = Observability.ActivitySource.StartMethodActivity(logger, () => new { taskFactories, parallelOptions });
        if (taskFactories?.Any() != true) { return null; }

        var taskFactoryList = taskFactories.ToList();
        var results = new ConcurrentDictionary<int, T>();

        await ForEachAsync(taskFactoryList.Select(static (factory, index) => new { Factory = factory, Index = index }),
            parallelOptions,
            async item =>
            {
    ...
    private const string headerName = "MaxConcurrency";
    private static void SetMaxConcurrency(ParallelOptions parallelOptions) // , CodeSectionScope scope
    {
        if (parallelOptions is null) { return; }
