...
namespace Common
{
    ...
    public static class CollectionExtensions
    {
        ...
        public static IEnumerable<T> ForEach<T>(this IEnumerable<T> source, Action<T> action)
        {
            ...
            try { enumerator = source.GetEnumerator(); } catch (Exception ex) { TraceManager.Exception(ex); }
            if (enumerator != null)
            {
                var ok = default(bool);
                try { ok = enumerator.MoveNext(); } catch (Exception ex) { TraceManager.Exception(ex); ok = false; }
                while (ok)
                {
